...
def test_execute_renders_table(mocker: MockerFixture) -> None:
    ...
    status_cells = list(rendered.columns[4].cells)
    assert isinstance(status_cells[1], Text)
    assert status_cells[1].plain == "no preference"
    assert status_cells[1].style == "yellow"
...
def test_execute_renders_no_preference_status(mocker: MockerFixture) -> None:
    """Status column should show no preference when overrides are absent."""
