"""Tests for the restoration use case orchestration."""
...
from omym.features.restoration.usecases.restore_music import RestorationService
from omym.platform.db.db_manager import DatabaseManager


class RestoreHarness(NamedTuple):
    """Bundle injected collaborators for restoration tests."""
...
@pytest.fixture()
def harness() -> RestoreHarness:
    """Provide a restoration service backed by an in-memory database."""

    manager = DatabaseManager(":memory:")
    repository = SqliteRestoreRepository(manager)
    filesystem = LocalFileSystemGateway()
    service = RestorationService(
        plan_reader=repository,
        filesystem=filesystem,
        maintenance=repository,
    )
    return RestoreHarness(service=service, db_manager=manager)
...
def test_restore_moves_file(tmp_path: Path, harness: RestoreHarness) -> None:
...
def test_restore_backup_collision(tmp_path: Path, harness: RestoreHarness) -> None:
...
def test_restore_dry_run(tmp_path: Path, harness: RestoreHarness) -> None:
...
def test_restore_moves_artwork(tmp_path: Path, harness: RestoreHarness) -> None:
...
def test_restore_artwork_collision_backup(tmp_path: Path, harness: RestoreHarness) -> None:
...
def test_restore_lyrics_collision_backup(tmp_path: Path, harness: RestoreHarness) -> None:
...
def test_restore_skips_when_source_matches_destination(
    tmp_path: Path, harness: RestoreHarness
) -> None:
